...
export default function LoginForm({ apiEndpoint = "/api/login" }) {
  ...
  return (
    <div className="mt-3 flex items-center justify-center bg-black-100 px-4 sm:px-6 md:px-8 min-h-screen">
      <form
        onSubmit={handleSubmit}
        noValidate
        className="bg-indigo-200 p-6 sm:p-8 m-4 sm:m-10 md:m-20 rounded-2xl shadow-xl w-full max-w-md sm:max-w-lg md:max-w-xl space-y-6 text-indigo-900"
      >
        ...
        <SubmitButton
          ...
          disabled={loading}
          loading={loading}
          className="w-full sm:w-3xs bg-indigo-600 hover:bg-indigo-700 text-white py-2 px-4 rounded-full transition-all duration-300 shadow-md hover:shadow-lg mt-3"
        >
          Submit
        </SubmitButton>
